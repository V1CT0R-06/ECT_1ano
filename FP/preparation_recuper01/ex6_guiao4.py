...
def sin_a(x):
    ...
    adding terms until the sum stops changing in double precision."""
    # sin x = x - x³/3! + x⁵/5! - x⁷/7! + ...
    s = x
    term = x
    k = 0
    while True:
        k += 1
        # recurrence: term_{k} = term_{k-1} * (-x^2) / ((2k)*(2k+1))
        term *= -x * x / ((2 * k) * (2 * k + 1))
        new_s = s + term
        if new_s == s:   # no further change in double precision
            break
        s = new_s
    return s
...
def sin(x):
    ...
    # Use periodicity and symmetries of the sine function to reduce the
